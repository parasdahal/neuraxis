...
$Function=new Utility;
  ...
  if(isset($_POST['iname'])&&isset($_POST['route'])&&isset($_POST['regparam'])&&isset($_FILES['dataset_file'])&&isset($_POST['alpha'])&&isset($_POST['num_iters']))
    { 
       $iname=$_POST['iname'];
       $route=$_POST['route'];
       
        $r_f_name=$_FILES['dataset_file']['name'];
        $r_name = $_SESSION['neuraxis'].'_log_'.$iname.'.csv';
      ...
      if ($upload_data==2 ) {
        header('Location:logistic.php?id=1');
      }
      
      else {
       $algo="LogisticRegressionClassifier";
       $iname=$_POST['iname'];
       $route=$_POST['route'];
       $regparam=(float)$_POST['regparam'];
       $alpha=(float)$_POST['alpha'];
       $num_iters=(int)$_POST['num_iters'];

        $datasets=array(array("name"=>"training","path"=>$r_name,"sanitizer"=>$sanitizer,"type"=>"csv"));
        ...
        $_SESSION['data']['regparam']=$regparam;
        $_SESSION['data']['alpha']=$alpha;
        $_SESSION['data']['num_iters']=$num_iters;
...
<div class="col-sm-8">
<h4>Please Select feautre and label Column from the dataset</h4><br/>
 <form class="form-horizontal" role="form" method="post" action="logistic_2.php" enctype="multipart/form-data">
          
   <div class="form-group">
            <label class="control-label col-sm-2" for="labelCol">Feature Column</label>
